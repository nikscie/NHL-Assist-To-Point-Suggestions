...
# cache file path
CACHE_FILE = f"nhl_cache_{SEASON}.json"
...
# compute hash of data for efficient comparison
def compute_hash(data):
    data_str = json.dumps(data, sort_keys=True)
    return hashlib.sha256(data_str.encode()).hexdigest()


# simple in-memory cache - loads from file when module is imported
cache = load_cache()
...
def get_team_roster(team_abbr, team_name):
    ...
    if url in cache:
        ...
        if cache[url] == data_hash:
            print(f"Roster unchanged for {team_name}, skipping")
            return None
        cache[url] = data_hash
        save_cache()
    else:
...
def get_team_schedule(team_abbr, team_name):
    ...
    if url in cache:
        ...
        data_hash = compute_hash(data)
        if cache[url] == data_hash:
            print(f"Schedule unchanged for {team_name}, skipping")
            return None
        cache[url] = data_hash
        save_cache()
    else:
    ...
    schedule_data = pandas.json_normalize(data, "games")
...
def get_team_goals(team_id, played_games):
    ...
    for _, game in played_games.iterrows():
        ...
        if url in cache:
            ...
            data_hash = compute_hash(data)
            if cache[url] == data_hash:
                # game unchanged, use cached result
...
def scrape():
    for team in teams:
        ...
        if played_games is None or played_games.empty:
            print(f"No new completed games for {name}")
            continue
        ...
        players_all = pandas.Series(
            pandas.concat([goals_df['scorer'], goals_df['assist1'], goals_df['assist2']])).dropna().unique()
